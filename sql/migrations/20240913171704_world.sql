...
-- Add hidden specialisation faction reputation from sniffs (used in various conditions)
UPDATE `quest_template` SET `RewRepFaction1` = 551, `RewRepValue1` = 75 WHERE `entry` IN (
3641,      -- Gnome Engineering Alliance
3643);     -- Gnome Engineering Horde

UPDATE `quest_template` SET `RewRepFaction1` = 550, `RewRepValue1` = 75 WHERE `entry` = 3639;     -- Goblin Engineering

UPDATE `quest_template` SET `RewRepFaction1` = 549, `RewRepValue1` = 200 WHERE `entry` IN (
5143,      -- Tribal Leatherworking Alliance
5148);     -- Tribal Leatherworking Horde

UPDATE `quest_template` SET `RewRepFaction1` = 86, `RewRepValue1` = 200 WHERE `entry` IN (
5141,      -- Dragonscale Leatherworking Alliance
5145);     -- Dragonscale Leatherworking Horde

UPDATE `quest_template` SET `RewRepFaction1` = 83, `RewRepValue1` = 200 WHERE `entry` IN (
5144,      -- Elemental Leatherworking Alliance
5146);     -- Elemental Leatherworking Horde

-- No reputation is rewarded for Weaponsmithing sub-specialisations (originally never implemented)
...
(11027, -1, 11003, 11024, 0, 0, 1),            -- NAND gate for Leatherworking and Engineering requirements for Book "Soothsaying for Dummies" gossip 
-- (If the conditions for both Leatherworking and Engineering gossips are met then the above condition allows to decide what gossip to display)
...
UPDATE `npc_text` SET `Probability0` = 1 WHERE `BroadcastTextID0` IN (
3283,      -- 1128
3284,      -- 1129
3285,      -- 1130
3286);     -- 1131
...
UPDATE `creature_template` SET `trainer_spell` = 0 WHERE `patch` = 0 AND `entry` IN (
...
7406,      -- Oglethorpe Obnoticus (Gnomish Engineering - Horde)
7866,      -- Peter Galen (Dragonscale Leatherworking - Alliance)
7867,      -- Thorkaf Dragoneye (Dragonscale Leatherworking - Horde)
7868,      -- Sarah Tanner (Elemental Leatherworking - Alliance)
7869,      -- Brumn Winterhoof (Elemental Leatherworking - Horde)
7870,      -- Caryssia Moonhunter (Tribal Leatherworking - Alliance)
7871,      -- Se'Jib (Tribal Leatherworking - Horde)
7944,      -- Tinkmaster Overspark (Gnomish Engineering - Alliance)
8126,      -- Nixx Sprocketspring (Goblin Engineering)
8738);     -- Vazario Linkgrease (Goblin Engineering)
...
UPDATE `gossip_menu_option` SET `condition_id` = 11037, `option_broadcast_text` = 4551 WHERE `menu_id` = 1465 AND `id` = 0;     -- Vazario Linkgrease (Goblin Engineering) + Correction to broadcast_text entry (currently uses 4553 which is the gnomish entry)
UPDATE `gossip_menu_option` SET `condition_id` = 11039 WHERE `menu_id` = 1467 AND `id` = 0;     -- Oglethorpe Obnoticus (Gnomish Engineering - Horde)
UPDATE `gossip_menu_option` SET `condition_id` = 11038 WHERE `menu_id` = 1468 AND `id` = 0;     -- Tinkmaster Overspark (Gnomish Engineering - Alliance)
UPDATE `gossip_menu_option` SET `condition_id` = 11037 WHERE `menu_id` = 1469 AND `id` = 0;     -- Nixx Sprocketspring (Goblin Engineering)
UPDATE `gossip_menu_option` SET `condition_id` = 11021 WHERE `menu_id` = 3067 AND `id` = 0;     -- Peter Galen (Dragonscale Leatherworking - Alliance)
...
UPDATE `quest_template` SET `RequiredMinRepFaction` = 289, `RequiredMinRepValue` = 6000  WHERE `entry` IN (
...
-- Update conditions for existing trainer gossip menu options
UPDATE `gossip_menu_option` SET `condition_id` = 1354 WHERE `id` = 0 AND `menu_id` IN (
597,       -- Grumnus Steelshaper (Armorsmithing - Alliance)
1043,      -- Shayis Steelfury (Armorsmithing - Horde)
3203);     -- Okothos Ironrage (Armorsmithing - Horde)

UPDATE `gossip_menu_option` SET `condition_id` = 1352 WHERE `id` = 0 AND `menu_id` IN (
1041,      -- Borgus Steelhand (Weaponsmithing - Alliance)
1042,      -- Kelgruk Blooadaxe (Weaponsmithing - Horde)
3201,      -- Ironus Coldsteel (Weaponsmithing - Alliance)
3202);     -- Borgosh Corebender (Weaponsmithing - Horde)
...
17039,      -- Swordsmithing
17040,      -- Hammersmithing
17041);     -- Axesmithing

-- Place Show Your Work quests in an Exclusive group to allow the unlocking of Membership Renewal Card on respecialisation
UPDATE `quest_template` SET `ExclusiveGroup` = 3600 WHERE `entry` IN (
3633,
3639,
3641);

-- Add profession spell condidtion to Membership Renewal Card quests
UPDATE `quest_template` SET `RequiredCondition` = 1367 WHERE `entry` IN (     -- Goblin Engineering
3644, 
3646);

UPDATE `quest_template` SET `RequiredRaces` = 178, `RequiredCondition` = 1368 WHERE `entry` = 3645;     -- Oglethorpe (Gnomish Engineering - Horde only)
UPDATE `quest_template` SET `RequiredCondition` = 1368 WHERE `entry` = 3647;     -- Gnomish Engineering
