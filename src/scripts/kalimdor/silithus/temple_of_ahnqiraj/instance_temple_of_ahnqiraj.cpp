...
void TwinsIntroDialogue::Start()
{
    ...
    if (!m_pInstance) {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: TwinsIntroDialogue missing instance. Cannot start.");
        return;
    }
    ...
    if (!pEye || !pVL || !pVN) {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Missing one or more of the required creatures to start TwinsIntroDialogue. Not starting.");
        if (pEye)
...
void TwinsIntroDialogue::JustDidDialogueStep(int32 iEntry)
{
    ...
    if (!pEye || !pVL || !pVN) {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Missing one or more of the required creatures in TwinsIntroDialogue::JustDidDialogueStep()");
        return;
    }
    switch (iEntry) {
...
void instance_temple_of_ahnqiraj::TeleportPlayerToCThun(Player* pPlayer)
{
    ...
    if (cthunAreaTrigger) {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: instance_temple_of_ahnqiraj::HandleStomachTriggers attempted to lookup area trigger %d, but it was not found.",
