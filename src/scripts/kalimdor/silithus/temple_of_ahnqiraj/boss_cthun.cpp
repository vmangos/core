...
struct cthunTentacle : public ScriptedAI
{
        ...
        ScriptedAI(pCreature)
    {
        ...
        if (!m_pInstance)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: C'thun tentacle could not find it's instance");
    ...
    bool UpdateCthunTentacle(uint32 diff)
    {
        ...
        if (!m_pInstance->GetPlayerInMap(true, false)) {
            if (TemporarySummon* tmpS = dynamic_cast<TemporarySummon*>(m_creature)) {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: CThunTentacle could not cast creature to TemporarySummon*");
            }
            //EnterEvadeMode();
...
public:
        ...
        groundRuptureTimer(pCreature, groundRuptSpellId, GROUND_RUPTURE_DELAY, 0, true, selectSelfFunc)
    {
        ...
        if (pPortal) {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: cthunPortalTentacle failed to spawn portal with entry %d", portalId);
        }
    }
    ...
    void DespawnPortal() {
        if (Creature* pCreature = m_creature->GetMap()->GetCreature(portalGuid)) {
            if (TemporarySummon* ts = dynamic_cast<TemporarySummon*>(pCreature)) {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Unable to despawn cthunPortalTentacle portal, could not cast to temporarySummon*");
            }
        }
    }
    ...
    void FixPortalPosition() {
        ...
        default:
            radius = 3.0f;
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: C'thun FixPortalPosition unknown portalID %d", portalEntry);
        }
...
struct clawTentacle : public cthunPortalTentacle
{
    ...
    bool UpdateClawTentacle(uint32 diff)
    {
        ...
        case BURRIED:
            ...
            break;
        default:
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Unknown UpdateClawTentacle state.");
            return false;
        }
    }
...
struct eye_of_cthunAI : public ScriptedAI
{
        ...
        ScriptedAI(pCreature)
    {
        ...
        m_pInstance = dynamic_cast<instance_temple_of_ahnqiraj*>(pCreature->GetInstanceData());
        if (!m_pInstance)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SD0: No Instance eye_of_cthunAI");
    ...
    void Reset() override
    {
        ...
        if (m_creature) {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: eye_of_cthunAI: Reset called, but m_creature does not exist.");
        }
    }
    ...
    void UpdateAI(uint32 const diff) override
    {
        ...
        case DARK_GLARE_COOLING:
            ...
            break;
        default:
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: CThun eye update called with incorrect state: %d", currentPhase);
        }
    }
...
struct cthunAI : public ScriptedAI
{
        ...
        wipeRespawnEyeTimer(0)
    {
        ...
        if (!m_pInstance)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SD0: No Instance for cthunAI");
    ...
    void AttackStart(Unit* who) override
    {
        if (!m_creature->IsInCombat()) {
            ...
            if (!pEye) {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: cthunAI::AggroRadius could not find pEye");
                return;
            }
    ...
    void CheckRespawnEye()
    {
        ...
        if (m_creature->IsDead()) {
            ...
            if (pEye = m_pInstance->GetCreature(eyeGuid)) {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: C'thun was unable to summon it's eye");
            }
        }
    }
    ...
    void JustSummoned(Creature* pCreature) override
    {
        if (pCreature->GetEntry() == MOB_FLESH_TENTACLE) {
            if (fleshTentacles.size() > 1) {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Flesh tentacle summoned, but there are already %i tentacles up.", fleshTentacles.size());
            }
    ...
    void UpdateAI(uint32 const diff) override
    {
        ...
        case PHASE_CTHUN_DONE:
            ...
            break;
        default:
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: C'Thun in bugged state: %i", currentPhase);
        }
    }
    ...
    void SpawnFleshTentacles() {
        ...
        if (!fleshTentacles.empty()) {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SpawnFleshTentacles() called, but there are already %i tentacles up.", fleshTentacles.size());
        }
        ...
        if (timer < diff) {
            if (Unit* target = SelectRandomAliveNotStomach(m_pInstance))
            {
                if (target->GetPositionZ() < -30.0f) {
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Cthun trying to spawn %i <-30.0f", id);
                }
                float x;
                float y;
                float z;
