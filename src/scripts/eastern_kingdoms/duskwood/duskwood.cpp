...
void Handle_NightmareCorruption(/*const*/ Player* player)
{
    ...
    if (!corrupter)
    {
        ...
        if (!corrupter)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Handle_NightmareCorruption: Could not summon creature %u for quest %u for player %s",
    ...
    void UpdateEscortAI(uint32 const uiDiff) override
    {
        if (!m_bLaunchChecked)
        {
            if (m_uiLaunchTimer < uiDiff)
            {
                ...
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [Duskwood.Stitches] Emergency launch.");
                    Start();
                }
...
void elloEbonlockeAI::LaunchStitches(Creature* pStitches) const
{
    if (auto stitchesAI = static_cast<npc_stitchesAI*>(pStitches->AI()))
    {
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [Duskwood.Stitches] Failed to cast AI.");
}
