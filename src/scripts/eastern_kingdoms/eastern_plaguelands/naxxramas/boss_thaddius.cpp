...
struct npc_tesla_coilAI : public Scripted_NoMovementAI
{
    ...
    void ReApplyChain(uint32 uiEntry, ObjectGuid guid)
    {
        ...
        if (uiEntry == NPC_FEUGEN)
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: npc_tesla_coilAI::ReApplyChain got entry which was not stalagg or feugen.");
    ...
    void HandleCheckSpawnAdd(eStalagFeugen whichAdd)
    {
        ...
        { 
            if (TemporarySummon* tmpSumm = static_cast<TemporarySummon*>(addCreature))
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Thaddius: HandleCheckSpawnAdd addCreature was not temp summon");
            addCreature = nullptr;
        }
        if(!addCreature)
        {
                ...
                TEMPSUMMON_MANUAL_DESPAWN))
            {
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Thaddius: failed spawning add %d", whichAdd);
        }
        ...
        if (coilCreature)
        {
            if (TemporarySummon* tmpSumm = static_cast<TemporarySummon*>(coilCreature))
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Thaddius: HandleCheckSpawnAdd coilCreature was not temp summon");
        }
        ...
        if (Creature* tc = m_creature->SummonCreature(NPC_TESLA_COIL, teslaCoilPositions[whichAdd][0], teslaCoilPositions[whichAdd][1], teslaCoilPositions[whichAdd][2], 0, TEMPSUMMON_MANUAL_DESPAWN))
        {
            ...
            if (npc_tesla_coilAI* pTeslaAI = dynamic_cast<npc_tesla_coilAI*>(tc->AI()))
                ...
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: boss_thaddiusAddsAI::EstablishLink failed to cast tesla coil to npc_tesla_coilAI*");
        }
        else
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: boss_thaddiusAddsAI::EstablishLink failed to spawn teslaCoil");
    ...
    void TransitionToPhase(ThaddiusPhase newPhase)
    {
        ...
        switch (m_Phase)
        {
            ...
            case ThaddiusPhase::THAD_PHASE2:
                ...
                break;
            default:
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: boss_thaddiusAI in undefined phase-state");
        }
    }
