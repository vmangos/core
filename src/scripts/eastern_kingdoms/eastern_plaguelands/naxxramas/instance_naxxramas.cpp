...
bool instance_naxxramas::HandleEvadeOutOfHome(Creature* pWho)
{
    ...
    switch (entry)
    {
        ...
        case NPC_THANE:
            ...
            break;
        default:
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: instance_naxxramas::HandleEvadeOutOfHome called for unsupported creture %d", pWho->GetEntry());
            dist = 9999.0f;
            break;
    }
...
void instance_naxxramas::UpdateAutomaticBossEntranceDoor(GameObject* pGO, uint32 uiData, int requiredPreBossData)
{
    ...
    if (!pGO)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: instance_naxxramas::UpdateAutomaticBossEntranceDoor called with nullptr GO");
        return;
    }
...
void instance_naxxramas::UpdateBossGate(GameObject* pGO, uint32 uiData)
{
    if (!pGO)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: instance_naxxramas::UpdateBossGate called with nullptr GO");
        return;
    }
    if (uiData == DONE)
        pGO->SetGoState(GO_STATE_ACTIVE);
...
void instance_naxxramas::UpdateTeleporters(uint32 uiType, uint32 uiData)
{
    ...
    switch (uiType)
    {
        ...
        case TYPE_FOUR_HORSEMEN:
            ...
            break;
        default:
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: instance_naxxramas::UpdateTeleporters called with unsupported type %d", uiType);
    }
    ...
    switch (uiType)
    {
        ...
        case TYPE_FOUR_HORSEMEN:
            ...
            else if (uiData == DONE)
            {
                ...
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: 4hm just died. Unable to find Argent Dawn faction for reputation ");
                }
            }
            break;
        case TYPE_PATCHWERK:
...
uint32 instance_naxxramas::GetData(uint32 uiType)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: instance_naxxramas::GetData() called with %d as param. %d is MAX_ENCOUNTERS", uiType, MAX_ENCOUNTER);
    return 0;
}
...
uint64 instance_naxxramas::GetGOUuid(NaxxGOs which)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: instance_naxxramas::GetGOUuid called with param %d, not found", which);
        return 0;
    }
    return it->second;
}
...
bool instance_naxxramas::IsInRightSideGothArea(Unit const* pUnit)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: left/right side check, Gothik combat area failed.");
    return true;
}
