...
struct boss_ragnarosAI : ScriptedAI
{
    ...
    void UpdateAI(uint32 const diff) override
    {
        ...
        if (IsBanished && m_uiAttackTimer < diff)
        {
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [MoltenCore.Ragnaros] Cast %u impossible.", SPELL_EMERGE_VISUAL);
        }
        else if (IsBanished)
        {
            ...
            if (m_uiMagmaBlastTimer < diff)
            {
                ...
                if (target)
                {
                    ...
                    if (!canCastResult)
                        ...
                        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [MoltenCore.Ragnaros] Magma Blast failed with reason <%u>.", canCastResult);

                    return;
                }

                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [MoltenCore.Ragnaros] No target to Magma Blast.");
            }
            else
    ...
    void CheckForMelee()
    {
            ...
            SELECT_FLAG_NOT_PLAYER | SELECT_FLAG_IN_LOS | SELECT_FLAG_IN_MELEE_RANGE))
        {
            ...
            //sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [MoltenCore.Ragnaros] Target type #4 reached with name <%s> and entry <%u>.", pTarget->GetName(), pTarget->GetEntry());
        }

        // nothing in melee at all
        //sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [MoltenCore.Ragnaros] CheckForMelee hits the end. Nothing in melee.");
    }
};
