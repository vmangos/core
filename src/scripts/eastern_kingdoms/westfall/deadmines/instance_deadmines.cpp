...
enum
{
    SPELL_DISTRACTING_PLAIN =   3603,
    SPELL_EJECT_SNEED       =   5141,
    SPELL_TERRIFY           =   7399,
};

struct boss_sneed_shredderAI : public ScriptedAI
{
    boss_sneed_shredderAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }

    uint32 m_uiDistracting_Timer;
    uint32 m_uiTerrify_Timer;
    ...
    {
        DoCastSpellIfCan(m_creature, SPELL_EJECT_SNEED);
    }
...
CreatureAI* GetAI_boss_sneed_shredder(Creature* pCreature)
{
    return new boss_sneed_shredderAI(pCreature);
}
