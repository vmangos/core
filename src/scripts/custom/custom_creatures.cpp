...
bool GossipHello_TeleportNPC(Player *player, Creature *_Creature)   
{
    ...
    if (player->GetTeam() == HORDE)
    {
        ...
        player->ADD_GOSSIP_ITEM(5, "------------------主要城市------------------"               , GOSSIP_SENDER_MAIN, 1);
        ...
        player->ADD_GOSSIP_ITEM(5, "---------古尔巴十竞技场-日常任务-----------" , GOSSIP_SENDER_MAIN, 4015);
        // player->ADD_GOSSIP_ITEM(5, "大陆 - 卡利姆多"     , GOSSIP_SENDER_MAIN, 6010);
        // player->ADD_GOSSIP_ITEM(5, "大陆 - 东部王国"     , GOSSIP_SENDER_MAIN, 6020);
    ...
    {
        //player->ADD_GOSSIP_ITEM(5, "PreTBC Mall"          , GOSSIP_SENDER_MAIN, 74);
        player->ADD_GOSSIP_ITEM(5, "------------------主要城市------------------"               , GOSSIP_SENDER_MAIN, 2);
...
void SendDefaultMenu_TeleportNPC(Player *player, Creature *_Creature, uint32 action)
{
    ...
    {
        case 1: // Cities [HORDE]
            player->ADD_GOSSIP_ITEM(5, "奥格瑞玛 - 部落主城"             , GOSSIP_SENDER_MAIN, 20);
        ...
        case 2: // Cities [ALLIANCE]
            player->ADD_GOSSIP_ITEM(5, "铁炉堡 - 联盟主城"             , GOSSIP_SENDER_MAIN, 24);
        ...
        case 3: // Starting Places [HORDE]
            player->ADD_GOSSIP_ITEM(5, "暗影坟墓"          , GOSSIP_SENDER_MAIN, 40);
            player->ADD_GOSSIP_ITEM(5, "试练谷"      , GOSSIP_SENDER_MAIN, 41);
            player->ADD_GOSSIP_ITEM(5, "那拉什营地"          , GOSSIP_SENDER_MAIN, 42);
            player->ADD_GOSSIP_ITEM(5, "<-[返回主菜单]"                  , GOSSIP_SENDER_MAIN, 100);

            player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, _Creature->GetGUID());
            break;
        case 4: // Starting Places [ALLIANCE]
            player->ADD_GOSSIP_ITEM(5, "北郡谷"     , GOSSIP_SENDER_MAIN, 43);
            player->ADD_GOSSIP_ITEM(5, "寒脊山谷"      , GOSSIP_SENDER_MAIN, 44);
            player->ADD_GOSSIP_ITEM(5, "影月谷"            , GOSSIP_SENDER_MAIN, 45);
            player->ADD_GOSSIP_ITEM(5, "<-[返回主菜单]"                  , GOSSIP_SENDER_MAIN, 100);
...
void Enchant(Player* player, Item* item, uint32 enchantid)
{
    if (!item)
    {
        player->GetSession()->SendNotification("你必须首先装备你要附魔的物品。");
        return;
    }

    if (!enchantid)
    {
        player->GetSession()->SendNotification("发生了一些错误。");
        return;
    }
...
bool GossipHello_EnchantNPC(Player* player, Creature* creature)
{
    ...
    player->ADD_GOSSIP_ITEM(5, "随便的?", GOSSIP_SENDER_MAIN, EQUIPMENT_SLOT_OFFHAND);
...
bool GossipSelect_EnchantNPC(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    if (action < 20)
    {
        ...
        case EQUIPMENT_SLOT_CHEST:
            player->ADD_GOSSIP_ITEM(5, "更大的数据", GOSSIP_SENDER_MAIN, CHEST_STATS);
            break;
        case EQUIPMENT_SLOT_BACK:
            ...
            player->ADD_GOSSIP_ITEM(5, "智力?", GOSSIP_SENDER_MAIN, CLOAK_SUB);
        break;
        case EQUIPMENT_SLOT_WRISTS:
            player->ADD_GOSSIP_ITEM(5, "耐力", GOSSIP_SENDER_MAIN, BRACER_STAM);
            player->ADD_GOSSIP_ITEM(5, "力量", GOSSIP_SENDER_MAIN, BRACER_STR);
            player->ADD_GOSSIP_ITEM(5, "精神?", GOSSIP_SENDER_MAIN, BRACER_HEAL);
            player->ADD_GOSSIP_ITEM(5, "智力", GOSSIP_SENDER_MAIN, BRACER_INT);
            break;
        case EQUIPMENT_SLOT_HANDS:
            player->ADD_GOSSIP_ITEM(5, "敏捷", GOSSIP_SENDER_MAIN, GLOVES_AGI);
            player->ADD_GOSSIP_ITEM(5, "火焰强化", GOSSIP_SENDER_MAIN, GLOVES_FIRE);
            player->ADD_GOSSIP_ITEM(5, "冰霜力量", GOSSIP_SENDER_MAIN, GLOVES_FROST);
            player->ADD_GOSSIP_ITEM(5, "暗影之力", GOSSIP_SENDER_MAIN, GLOVES_SHADOW);
            player->ADD_GOSSIP_ITEM(5, "疗伤", GOSSIP_SENDER_MAIN, GLOVES_HEALING);
            break;
        case EQUIPMENT_SLOT_FEET:
            player->ADD_GOSSIP_ITEM(5, "耐力", GOSSIP_SENDER_MAIN, BOOTS_STAM);
            player->ADD_GOSSIP_ITEM(5, "较小的速度?", GOSSIP_SENDER_MAIN, BOOTS_SPEED);
        ...
        case EQUIPMENT_SLOT_MAINHAND:
            ...
            player->ADD_GOSSIP_ITEM(5, "1H 敏捷", GOSSIP_SENDER_MAIN, WEP1H_AGILITY);
            player->ADD_GOSSIP_ITEM(5, "2H 敏捷", GOSSIP_SENDER_MAIN, WEP2H_AGILITY);
            player->ADD_GOSSIP_ITEM(5, "法术强度", GOSSIP_SENDER_MAIN, WEP_SPELLPOWER);
            player->ADD_GOSSIP_ITEM(5, "疗伤", GOSSIP_SENDER_MAIN, WEP_HEAL);
            break;
...
bool LearnAllRecipesInProfession(Player *pPlayer, SkillType skill)
{
    ...
    if (!SkillInfo)
    {
        sLog.outError("职业NPC:获得无效技能ID");
        return false;
    }

    pPlayer->SetSkill(SkillInfo->id, 300, 300);
    LearnSkillRecipesHelper(pPlayer, SkillInfo->id);
    pPlayer->GetSession()->SendNotification("所有配方 %s 以学会", skill_name);
    return true;
}
...
void CompleteLearnProfession(Player *pPlayer, Creature *pCreature, SkillType skill)
{
    ...
    {
        pPlayer->GetSession()->SendNotification("你已经学会了两个主要的职业。");
    }
    else
    {
