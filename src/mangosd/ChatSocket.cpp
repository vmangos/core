...
int OfflineChatSocket::open(void* )
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: OfflineChatSocket::open: unable to register client handler errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: OfflineChatSocket::open: peer ().get_remote_addr errno = %s", ACE_OS::strerror (errno));
        return -1;
    }
    return 0;
}
...
int OfflineChatSocket::handle_output (ACE_HANDLE)
{
    ...
    if (!outputBufferLen)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: OfflineChatSocket::handle_output: error while cancel_wakeup");
            return -1;
        }
        outActive = false;
        return 0;
...
int OfflineChatSocket::handle_input(ACE_HANDLE)
{
    ...
    if (closing_)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Called OfflineChatSocket::handle_input with closing_ = true");
        return -1;
    }
...
int OfflineChatSocket::sendf(const char* msg)
{
    ...
    if (!outActive)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: OfflineChatSocket::sendf error while schedule_wakeup");
            return -1;
        }
        outActive = true;
    }
