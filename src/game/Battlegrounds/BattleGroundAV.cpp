...
{
sLog.outDebug("BattleGroundAV: TeamIdx %i ", teamIdx);
    SpawnEvent(AV_05_TAMED+teamIdx, 2, true, false);
    SpawnEvent(AV_10_TAMED+teamIdx, 2, true, false);
    SpawnEvent(AV_15_TAMED+teamIdx, 2, true, false);
    SpawnEvent(AV_20_TAMED+teamIdx, 2, true, false);
...
void BattleGroundAV::EndBattleGround(Team winner)
{
    ...
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
        ...
        sLog.outDebug("BattleGroundAV: EndbattleGround: bgteam: %u towers:%u honor:%u rep:%u", i, towersSurvived[i], GetBonusHonorFromKill(towersSurvived[i] * BG_AV_KILL_SURVIVING_TOWER), towersSurvived[i] * BG_AV_REP_SURVIVING_TOWER);
        if (gravesOwned[i])
        {
...
void BattleGroundAV::EventPlayerClickedOnFlag(Player* source, GameObject* targetGo)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
    sLog.outDebug("BattleGroundAV: using gameobject %i", targetGo->GetEntry());
