...
void SpellAuraHolder::_AddSpellAuraHolder()
{
    ...
    if (IsNeedVisibleSlot(caster))
    {
        ...
        else                                                // empty negative slot
        {
            ...
            {
                if (i < MAX_AURAS && m_target->GetUInt32Value((uint16)(UNIT_FIELD_AURA + i)) == 0)
                {
                    slot = i;
                    break;
                }
                if (i == MAX_AURAS && !m_target->IsPlayer() && !m_target->IsPet()) // Wallcraft - 40 debuff slots
                {
                    for (uint8 j = 8; j < MAX_POSITIVE_AURAS; j++)
                    {
                        if (m_target->GetUInt32Value((uint16)(UNIT_FIELD_AURA + j)) == 0)
                        {
                            slot = j;
                            break;
                        }
...
/* Vanilla Reforged Debuff Categories:
 * 6 (highest)  NPC Abilities, Threat, Hard Crowd-Control
 * 5            Soft Crowd-Control
 * 4            Generally, special cases for raid viability
 * 3            Generally, debuffs with raid wide benefits
 * 2            Generally, debuffs with group wide benefits (Vampiric Embrace)
 * 1            Generally, primary dots and channels
 * 0            Generally, secondary dots
 * %            Triggered Spells are never higher than 2 and channeled spells never lower than 1 */
void SpellAuraHolder::CalculateForDebuffLimit()
{
    m_visibleSlotLimitAffected = true;
    m_visibleSlotLimitScore = 0;

    // determine based on spell id
    ...
    case 20185: // Judgement of Light
    case 20186: // Judgement of Wisdom
                // Category 2
                // No specific IDs for category 2, handled by aura type
                // Category 1
                // No specific IDs for category 1, handled by aura type
                //         // Category 0
    case 133: // Fireball
    ...
    for (uint32 i : m_spellProto->EffectApplyAuraName)
    {
        ...
        case SPELL_AURA_AOE_CHARM:
            currEffectScore = 6;
            break;
        case SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK: // Mind Numbing Poison, Curse of Tongues
        ...
        case SPELL_AURA_MOD_HEALING: // Wound Poison
            currEffectScore = 5;
            break;
        case SPELL_AURA_RANGED_ATTACK_POWER_ATTACKER_BONUS: // Expose Weakness
        case SPELL_AURA_MOD_RESISTANCE:
        case SPELL_AURA_MOD_STALKED: // Hunter's Mark
