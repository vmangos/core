    ...
    for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch (m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
                ...
                // aggro surrounding mobs when opening / lockpicking a gameobject with the faction "Treasure"
                ...
                if (strict)
                {
                    if (GameObject* go = m_targets.getGOTarget())
                    {
                        ...
                        if (sObjectMgr.GetFactionTemplateEntry(factionId)->faction == 77) // "Treasure" faction Id
