...
SpellSpecific Spells::GetSpellSpecific(uint32 spellId)
{
    ...
    if (spellInfo->HasSingleAura(SPELL_AURA_MOD_DECREASE_SPEED) && !(spellInfo->AttributesEx & SPELL_ATTR_EX_STOPS_AUTO_ATTACK))
        return SPELL_SNARE;
...
{
    if (!(Attributes & SPELL_ATTR_HEARTBEAT_RESIST_CHECK))
        return false;
