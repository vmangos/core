...
DumpReturn PlayerDumpReader::LoadDump(std::string const& file, uint32 account, std::string name, uint32 guid)
{
    ...
    while (!feof(fin))
    {
        ...
        if (tn.empty())
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: LoadPlayerDump: Can't extract table name from line: '%s'!", line.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        if (!dTable->isValid())
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: LoadPlayerDump: Unknown table: '%s'!", tn.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        switch (type)
        {
            ...
            default:
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Unknown dump table type: %u", type);
                break;
        }
