...
bool Creature::IsTrainerOf(Player* pPlayer) const
{
    ...
    if ((!cSpells || cSpells->spellList.empty()) && (!tSpells || tSpells->spellList.empty()))
	 {
        sLog.Out(LOG_DBERROR, LOG_LVL_MINIMAL, "Creature %u (Entry: %u) has npc_flag UNIT_NPC_FLAG_TRAINER but either has no trainer spell list or the list is empty.",
                        GetGUIDLow(), GetEntry());
        return false;
    }
	...
	// Get gossip menu options for this NPC
	std::pair<GossipMenuItemsMap::iterator, GossipMenuItemsMap::iterator> bounds = 
