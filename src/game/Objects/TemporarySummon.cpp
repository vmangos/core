...
void TemporarySummon::Update(uint32 update_diff,  uint32 diff)
{
    ...
    if (GetCharmerGuid().IsEmpty() || !HasAuraType(SPELL_AURA_MOD_CHARM))
    {
        switch (m_type)
        {
            ...
            default:
                UnSummon();
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Temporary summoned creature (entry: %u) have unknown type %u of ", GetEntry(), m_type);
                break;
        }
    }
...
TemporarySummon::~TemporarySummon()
{
    ...
    if (!m_unSummonInformed)
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: TemporarySummon %s deleted before being unsummed - summoner will retain incorrect count", GetGuidStr().c_str());
}
