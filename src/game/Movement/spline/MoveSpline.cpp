...
void MoveSpline::init_spline(MoveSplineInitArgs const& args)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: MoveSpline::init_spline: zero length spline, wrong input data?");
...
#define CHECK(exp) \
    if (!(exp))\
    {\
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: MoveSplineInitArgs::Validate: expression '%s' failed for %s", #exp, unit->GetGuidStr().c_str());\
        return false;\
    }
    CHECK(path.size() > 1);
    CHECK(velocity > 0.f);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size() - 1; ++i)
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
