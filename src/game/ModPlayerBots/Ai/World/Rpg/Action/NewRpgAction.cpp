...
bool TellRpgStatusAction::Execute(Event event)
{
    ...
    bot->MonsterWhisper(out.c_str(), owner);
    return true;
}
...
bool StartRpgDoQuestAction::Execute(Event event)
{
    ...
    if (quest)
    {
        ...
        bot->MonsterWhisper(("Start to do quest " + std::to_string(questId)).c_str(), owner);
        return true;
    }
    bot->MonsterWhisper(("Invalid quest " + text).c_str(), owner);
    return false;
}
...
bool NewRpgWanderNpcAction::Execute(Event event)
{
    ...
    WorldObject* object = bot->GetMap()->GetWorldObject(info.wander_npc.npcOrGo);
...
bool NewRpgTravelFlightAction::Execute(Event event)
{
    ...
    Creature* flightMaster = bot->GetMap()->GetCreature(botAI->rpgInfo.flight.fromFlightMaster);
