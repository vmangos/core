...
void WardenScanMgr::loadFromDB()
{
    ...
    do
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Unknown Warden scan type %u.  Skipped.", scanType);
            continue;
        }
        ...
        switch (scanType)
        {
            case READ_MEMORY:
            {
                ...
                {
                    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Failed to parse expected value in Warden scan id %u", id);
                    continue;
                }
            ...
            default:
            {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Unhandled Warden scan type %u id %u.  Skipped.", scanType, id);
                continue;
            }
        }
        ...
        if (!scan)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Failed to allocate Warden scan type %u id %u", scanType, id);
            continue;
        }
        else
        {
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, ">> %u Warden scans loaded from world database", m_scans.size());
}
