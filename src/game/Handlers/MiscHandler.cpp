...
{
    sLog.outDetail("WORLD: Received opcode CMSG_UPDATE_ACCOUNT_DATA");

    uint32 type, decompressedSize;
    recv_data >> type >> decompressedSize;

    sLog.outDetail("UAD: type %u, decompressedSize %u", type, decompressedSize);
...
{
    sLog.outDetail("WORLD: Received opcode CMSG_REQUEST_ACCOUNT_DATA");

    uint32 type;
    recv_data >> type;

    sLog.outDetail("RAD: type %u", type);
