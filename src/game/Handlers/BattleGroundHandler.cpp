...
void WorldSession::HandleBattlemasterHelloOpcode(WorldPacket& recv_data)
{
    ...
    sLog.outDebug("WORLD: Recvd CMSG_BATTLEMASTER_HELLO Message from %s", guid.GetString().c_str());
...
void WorldSession::HandleBattlefieldJoinOpcode(WorldPacket& recv_data)
{
    uint32 mapId;
    recv_data >> mapId;
    sLog.outDebug("WorldSession::HandleBattlefieldJoinOpcode : mapId=%u", mapId);
...
{
    sLog.outDebug("WORLD: Recvd CMSG_BATTLEMASTER_JOIN");
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket& recv_data)
{
    ...
    if (bgTypeId == BATTLEGROUND_TYPE_NONE)
    {
        sLog.outDebug("BattlegroundHandler: invalid bg map (%u) received.", mapId);
        return;
    }
    ...
    {
        sLog.outDebug("BattlegroundHandler: itrplayerstatus not found.");
        return;
    }
    // if action == 1, then instanceId is required
    if (!ginfo.isInvitedToBgInstanceGuid && action == 1)
    {
        sLog.outDebug("BattlegroundHandler: instance not found.");
        return;
    }
    ...
    if (!bg)
    {
        sLog.outDebug("BattlegroundHandler: bgTemplate not found for type id %u.", bgTypeId);
        return;
    }
