...
void DragonsOfNightmare::Update()
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [Dragons of Nightmare] Only %u nightmare dragons exist in the database, there should be 4", dragonGUIDs.size());
        return;
    }
...
void DragonsOfNightmare::CheckSingleVariable(uint32 idx, uint32& value)
{
    ...
    if (!variableExists)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: GameEventMgr: [Dragons of Nightmare] variable does not exist! Setting default.");
...
void DragonsOfNightmare::GetAliveCountAndUpdateRespawnTime(std::vector<ObjectGuid>& dragons, uint32& alive, time_t respawnTime)
{
    for (auto& guid : dragons)
    {
        ...
        if (!cData)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: GameEventMgr: [Dragons of Nightmare] creature data %u not found!", guid.GetCounter());
            continue;
        }
        ...
        if (!map)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: GameEventMgr: [Dragons of Nightmare] instance %u of map %u not found!", instanceId, cData->position.mapId);
            continue;
        }
        ...
        if (!pCreature)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: GameEventMgr: [Dragons of Nightmare] creature %u not found!", guid.GetCounter());
            continue;
        }
...
bool DragonsOfNightmare::LoadDragons(std::vector<ObjectGuid>& dragonGUIDs)
{
    for (uint32 entry : NightmareDragons)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: GameEventMgr: [Dragons of Nightmare] creature %u not found in world!", entry);
            return false;
        }
...
Map* ScourgeInvasionEvent::GetMap(uint32 mapId, Position const& invZone)
{
    ...
    if (!pMap)
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::GetMap found no map with mapId %d, x: %d, y: %d.", mapId, invZone.x, invZone.y);
    return pMap;
}
...
void ScourgeInvasionEvent::HandleActiveZone(uint32 attackTimeVar, uint32 zoneId, uint32 remainingVar, time_t now)
{
    ...
    else if (t < now && sObjectMgr.GetSavedVariable(remainingVar) == 0)
    {
        ...
        if (pMouth)
            ...
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::HandleActiveZone ObjectGuid %d not found.", zone->mouthGuid);
    }
}
...
bool ScourgeInvasionEvent::OnEnable(uint32 zoneId, uint32 attackTimeVar)
{
    ...
    else
    {
        ...
        {
            if (!oldZone)
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::OnEnable starting new invasion as oldZone could not be found.");
            StartNewInvasionIfTime(attackTimeVar, zoneId);
        }
    }
    return true;
...
void ScourgeInvasionEvent::StartNewCityAttackIfTime(uint32 timeVariable, uint32 zoneID)
{
    ...
    if (!mapPtr)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::StartNewCityAttackIfTime unable to access required map (%d). Retrying next update.", zone->map);
        return;
    }
    ...
    if (mapPtr && SummonPallid(mapPtr, zone, zone->pallid[SpawnLocationID], SpawnLocationID))
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::StartNewCityAttackIfTime unable to spawn pallid in %d.", zone->map);
}
...
void ScourgeInvasionEvent::StartNewInvasionIfTime(uint32 timeVariable, uint32 zoneId)
{
    ...
    if (!mapPtr)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::StartNewInvasionIfTime unable to access required map (%d). Retrying next update.", zone->map);
        return;
    }
    ...
    if (mapPtr && SummonMouth(mapPtr, zone, zone->mouth[0]))
        ...
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::StartNewInvasionIfTime unable to spawn mouth in %d.", zone->map);
}
...
bool ScourgeInvasionEvent::ResumeInvasion(uint32 zoneId)
{
    ...
    InvasionZone* zone = GetInvasionZone(zoneId);
    if (!zone) {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::ResumeInvasion somehow magically could not find InvasionZone object for zoneId: %d.", zoneId);
        return false;
    }
    ...
    for (uint32 i = 0; i < num_necropolises_remaining; i++)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::ResumeInvasion map %d not accessible. Retry next update.", zone->map);
            return false;
        }
    }
    ...
    if (!mapPtr)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::ResumeInvasion failed getting map, even after making sure they were loaded....");
        return false;
    }
...
bool ScourgeInvasionEvent::SummonPallid(Map* pMap, CityAttack* zone, Position position, uint32 SpawnLocationID)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::SummonPallid failed summoning Pallid Horror.");
        return false;
    }
...
bool ScourgeInvasionEvent::SummonMouth(Map* pMap, InvasionZone* zone, Position position)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::SummonMouth failed summoning Mouth of Kel'Thuzad.");
        return false;
    }
...
bool ScourgeInvasionEvent::isActiveZone(uint32 zoneId)
{
    for (const auto& invasionPoint : invasionPoints)
    {
        if (invasionPoint.zoneId == zoneId)
        {
            ...
            if (!mapPtr)
            {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::isValidZoneId no map for zone %d.", invasionPoint.map);
                continue;
            }
...
uint32 ScourgeInvasionEvent::GetActiveZones()
{
    ...
    for (const auto& invasionPoint : invasionPoints)
    {
        ...
        if (!mapPtr)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::GetActiveZones no map for zone %d.", invasionPoint.map);
            continue;
        }
...
ScourgeInvasionEvent::CityAttack* ScourgeInvasionEvent::GetCityZone(uint32 zoneId)
{
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: ScourgeInvasionEvent::GetZone unknown zoneid: %d.", zoneId);
    return nullptr;
}
...
void WarEffortEvent::Update()
{
    ...
    switch (stage)
    {
        ...
        default:
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [WarEffortEvent] Stuck in invalid stage %u", stage);
            break;
        }
    }
...
void WarEffortEvent::UpdateStageEvents()
{
    ...
    for (const auto iter : required)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: [WarEffortEvent] Event %u is already active for stage %u, but not defined in overall event list",
                iter, stage);
        }
    }
}
