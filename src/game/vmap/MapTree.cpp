...
namespace VMAP
{
    ...
    */
            ...
            if (!iIsTiled)
            {
                ...
                while (ModelSpawn::readFromFile(rf, spawn))
                {
                    ...
                    if (model)
                        ...
                        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: StaticMapTree::LoadMapTile() could not acquire WorldModel pointer for '%s'!", spawn.name.c_str());
                    ...
                    if (!iLoadedSpawns.count(referencedVal))
                    {
                        ...
                        {
                            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: invalid tree element! (%u/%u)", referencedVal, iNTreeValues);
                            continue;
                        }
    ...
    bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        if (!iTreeValues)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: StaticMapTree::LoadMapTile(): Tree has not been initialized! [%u,%u]", tileX, tileY);
            return false;
        }
        bool result = true;
    ...
    void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: StaticMapTree::UnloadMapTile(): Trying to unload non-loaded tile. Map:%u X:%u Y:%u", iMapID, tileX, tileY);
            return;
        }
                ...
                for (uint32 i = 0; i < numSpawns && result; ++i)
                {
                    ...
                    if (result)
                    {
                        ...
                        if (!iLoadedSpawns.count(referencedNode))
                            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Trying to unload non-referenced model '%s' (ID:%u)", spawn.name.c_str(), spawn.ID);
