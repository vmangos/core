...
bool ChatHandler::HandleCharacterHasItemCommand(char* args)
{
    ...
    else
    {
        std::unique_ptr<QueryResult> result(CharacterDatabase.PQuery(
            "SELECT SUM(`count`) AS `item_count` FROM `item_instance` WHERE `itemEntry` = '%u' and `owner_guid` = '%u'",
    ...
    else
    {
        ...
        while (stacksToRemove)
        {
            result.reset(CharacterDatabase.PQuery(
                "SELECT `guid`, `count` FROM `item_instance` WHERE `itemEntry` = '%u' and `owner_guid` = '%u' ORDER BY `count` DESC",
                itemId, target_guid.GetCounter()
            ));
