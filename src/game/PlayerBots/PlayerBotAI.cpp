...
bool PlayerBotAI::SpawnNewPlayer(WorldSession* sess, uint8 class_, uint32 race_, uint32 mapId, uint32 instanceId, float x, float y, float z, float o, Player* pClone)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: PlayerBotAI::SpawnNewPlayer: Unable to create a player!");
        delete newChar;
        return false;
    }
    ...
    if (!map)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: PlayerBotAI::SpawnNewPlayer: Map (%u, %u) not found!", mapId, instanceId);
        delete newChar;
        return false;
    }
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: PlayerBotAI::SpawnNewPlayer: Unable to add player to map!");
        delete newChar;
        return false;
    }
