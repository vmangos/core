...
void SQLStorageLoaderBase<DerivedLoader, StorageClass>::Load(StorageClass& store, bool error_at_empty /*= true*/)
{
    ...
    if (!result)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Error loading %s table (not exist?)\n", store.GetTableName());
    ...
    {
        if (error_at_empty)
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: %s table is empty!\n", store.GetTableName());
        else
    ...
    {
        recordCount = 0;
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: Error in %s table, probably sql file format was updated (there should be %d fields in sql).\n", store.GetTableName(), store.GetSrcFieldCount());
        delete result;
        Log::WaitBeforeContinueIfNeed();
