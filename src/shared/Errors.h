...
/// </example>
#define MANGOS_ASSERT(mustBeTrue) do { if (!(mustBeTrue)) { MaNGOS::Errors::PrintStacktraceAndThrow(__FILE__, __LINE__, __FUNCTION__, MANGOS_ERROR_STRING_ESCAPE(condition)); } } while(0)

#ifdef MANGOS_DEBUG
#define MANGOS_DEBUG_ASSERT(mustBeTrue) MANGOS_ASSERT(mustBeTrue)
#else
#define MANGOS_DEBUG_ASSERT(mustBeTrue) do {} while(0)
#endif
